...
SUPERLATIVE_INSTRUCTIONS = """최상급 표현: 허위사실로 오인될 수 있는 표현
   ...
   - "가장 X" 표현은 X가 높이·크기·수량 등 객관적으로 측정 가능한 속성일 때만 검출 (예: 가장 높은, 가장 낮은, 가장 넓은). X가 안전·우수 등 주관적 평가 속성이거나, 위 제외 목록에 있는 표현(예: 최근, 최선)이면 검출 대상에서 제외 (예: 가장 안전한, 가장 우수한, 가장 최근의)
   - 아래 표현은 검출 대상에서 반드시 제외할 것: 선도, 이해도 100%, 최적, 최적화, 최대 OOO(뒤에 구체적인 수치·단위가 붙는 "최대" 표현, 예: 최대 200명, 최대 30%), 극대화, 최대한, 최우선, 최선, 최신, 최근, 탁월, 탁월한, 독보적, 압도적
