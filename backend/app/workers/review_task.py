...
SUPERLATIVE_INSTRUCTIONS = """최상급 표현: 허위사실로 오인될 수 있는 표현
   ...
   - "최고 X", "최고의 X", "가장 X" 표현은 X가 높이·크기·수량·순위·속도 등 객관적으로 측정 가능한 속성일 때만 검출 (예: 최고 시속, 최고 순위, 가장 높은, 가장 낮은, 가장 넓은).
     X가 안전·우수·효율·전문성 등 주관적 평가 속성이면 (여러 속성이 "~하고 ~한" 형태로 나열된 경우 포함) 검출 대상에서 제외
     (예: 최고의 전문가 조직, 가장 안전한, 가장 안전하고 막힘없는, 가장 안전하고 효율적인, 가장 최근의)
...
def _is_excluded_superlative(detected_text: str) -> bool:
    """LLM이 시스템 프롬프트의 제외 지시를 놓치는 경우가 있어, 확정적으로 제외해야 하는
    표현(고정 단어·최대+숫자 패턴)은 코드에서 한 번 더 걸러낸다. "최고 X"/"가장 X"처럼
    주관적/객관적 판단이 필요한 항목은 LLM 판단에 맡기고 여기서는 다루지 않는다."""
