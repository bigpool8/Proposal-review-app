...
SUPERLATIVE_INSTRUCTIONS = """최상급 표현: 허위사실로 오인될 수 있는 표현
   - 해당 표현: 최초, 최대, 최고, 유일, 가장, 세계 최
   - "최고 X", "최고의 X", "가장 X" 표현은 X가 높이·크기·수량·순위·속도 등 객관적으로 측정 가능한 속성일 때만 검출 (예: 최고 시속, 최고 순위, 가장 높은, 가장 낮은, 가장 넓은).
     X가 안전·우수·효율·전문성·역량 등 주관적 평가 속성이면 (여러 속성이 "~하고 ~한" 형태로 나열된 경우 포함) 검출 대상에서 제외
     (예: 최고의 전문가 조직, 최고의 인력, 최고의 기술력, 가장 안전한, 가장 안전하고 막힘없는, 가장 안전하고 효율적인, 가장 최근의)
   - "세계 최"는 반드시 "세계" 바로 뒤에 "최"가 이어지는 경우만 해당 (예: 세계 최초, 세계 최대, 세계 최고, 세계 유일).
     "세계"로 시작하지만 뒤에 "최"가 없는 고유명사·행사명·기관명은 절대 검출하지 말 것 (예: 세계총회, 세계은행, 세계보건기구, 세계무역기구, 세계경제포럼)
   - "최대" 바로 뒤에 구체적인 수치(숫자)가 붙어 그 수치의 상한을 나타내는 표현은 단위 종류와 무관하게 모두 검출 대상에서 제외 (예: 최대 200명, 최대 30%, 최대 4차로, 최대 300MW, 최대 10km, 최대 5년 등 숫자+단위 조합 전체)
   - 아래 표현은 검출 대상에서 반드시 제외할 것: 선도, 선도적, 이해도 100%, 최적, 최적화, 최적화된, 극대화, 최대한, 최우선, 최선, 최신, 최근, 탁월, 탁월한, 독보적, 압도적, 완벽, 완벽한
   - 위 "해당 표현" 목록(및 그 활용형)에 없는 단어는 아무리 강조하는 느낌이더라도 최상급 표현으로 검출하지 말 것 (예: 완성, 세계 속의, 최상, 뛰어난 등은 목록에 없으므로 검출 대상이 아님).
     TOP, No.1, BEST, 1위처럼 의미가 비슷해 보이는 영어·숫자 표현도 위 목록의 한국어 단어가 실제로 쓰인 경우가 아니면 검출하지 말 것
   - 해당 표현이 있어도 사실일 수 있으므로 '검토 필요'로만 플래그. 오류로 단정하지 말 것.
   - 고유명사 일부로 포함된 경우(예: 최고급 상품명)는 제외
   - category는 "superlative", suggestion은 항상 null"""

_SUPERLATIVE_EXCLUDE_SUBSTRINGS = [
    "선도", "이해도 100%", "최적", "극대화", "최대한", "최우선", "최선",
    "최신", "최근", "탁월", "독보적", "압도적", "완벽",
]
_SUPERLATIVE_EXCLUDE_NUMERIC_RE = re.compile(r"최대\s*\d")
_SUPERLATIVE_REQUIRED_VOCAB_RE = re.compile(r"최초|최대|최고|유일|가장|세계\s*최")


def _has_required_superlative_keyword(detected_text: str) -> bool:
    """detected_text가 SUPERLATIVE_INSTRUCTIONS에 명시된 6개 표현
    (최초/최대/최고/유일/가장/세계+최, 및 그 활용형)을 실제로 포함하는지 확인한다.
    LLM이 지정된 어휘 목록을 벗어나 의미상 비슷한 표현(TOP, 세계총회처럼 '세계'만
    있고 '최'가 이어지지 않는 고유명사 등)을 잘못 검출하는 경우를 걸러내기 위한
    순수 어휘 존재 여부 체크이며, 주관/객관 판단과는 무관하다."""
    return bool(_SUPERLATIVE_REQUIRED_VOCAB_RE.search(detected_text))


def _is_excluded_superlative(detected_text: str) -> bool:
    """LLM이 시스템 프롬프트의 제외 지시를 놓치는 경우가 있어, 확정적으로 제외해야
    하는 항목은 코드에서 한 번 더 걸러낸다:
    1) 지정된 6개 단어(및 활용형) 자체가 detected_text에 전혀 없는 경우
       (TOP, 세계총회 등 LLM이 의미 유사도로 어휘 목록을 벗어나 검출한 경우)
    2) 고정 제외 단어 목록에 해당하는 경우
    3) "최대"+숫자로 상한을 나타내는 경우
    "최고 X"/"가장 X"에서 X가 주관적 속성인지 객관적 속성인지 같은 판단은 여전히
    LLM에게 맡기고 여기서는 다루지 않는다 — 위 1)은 어휘의 유무만 보는 것이지
    주관/객관을 판단하는 것이 아니다."""
    if not _has_required_superlative_keyword(detected_text):
        return True
    if any(term in detected_text for term in _SUPERLATIVE_EXCLUDE_SUBSTRINGS):
        return True
